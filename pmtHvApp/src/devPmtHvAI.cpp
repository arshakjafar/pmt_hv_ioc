...
/*Create the dset for devPmtHvAI */
...
static long init_record(void* pRecord)
{
	...
	if(pAi->inp.type == INST_IO)
	{
		...
		PmtHvRecordDriver* recordDriver = new PmtHvRecordDriver(pAi->inp.value.instio.string, pcallback);
		pAi->dpvt = recordDriver;
		// printf("Record inp: inst_io type, value: %s.\n", pAi->inp.value.instio.string);
	}
	else
	{
		recGblRecordError(S_db_badField, (void*)pAi, "devPmtHvAI (init_record) Illegal INP field.");
...
{
    struct aiRecord	*pAi = (struct aiRecord	*)pRecord;
	PmtHvRecordDriver* recordDriver = (PmtHvRecordDriver*)pAi->dpvt;
	if(pAi->pact)
	{
		epicsFloat64 val = 0;
